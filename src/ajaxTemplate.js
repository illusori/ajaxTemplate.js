...
applyAction: function ( fragmentNode, actionDef, valOb )
{
    ...
    {
        if( prop = actionDef.getAttribute( 'ajt:' + this.validActions[ i ] ) )
        {
...
//alert( 'style search for ' + ot + ' found ' + targets );
    ...
    {
        //  Default target is span/div with same ajt:name attribute as prop
        var attr = '[ajt:name=\'' + prop + '\']';
        targets = fragmentNode.select( 'SPAN' + attr + ', DIV' + attr );
...
function ajtTest( containerID )
{
    var valOb = {
        eveuser_id: 999,
        register_date: 'register date goes here',
        first_fetched: 'first_fetched date goes here',
        limited_apikey: 'aaaabbbbccccddddeeeeffff'
        };

    var newChild =
        ajaxTemplate.copyTemplate( containerID, 'eveuser_account', valOb );
    ...
    $(containerID).select( '.accountlist' )[ 0 ].appendChild( newChild );
}
